...
To generate a testcase, use:
    sc-issue -generate -cfg <stepdir>/outputs/<design>.pkg.json.gz

    or include a different step/index than what the cfg_file is pointing to:
    sc-issue -generate -cfg <otherdir>/outputs/<design>.pkg.json.gz -arg_step <step> -arg_index <index>

    or include specific libraries while excluding others:
    sc-issue -generate -cfg <stepdir>/outputs/<design>.pkg.json.gz -exclude_libraries -add_library sram -add_library gpio
